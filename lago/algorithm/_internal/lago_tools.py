...
def update_fast_iteration_exploration_set(
    child_module: _LagoModule, exploration_set: set[_LagoModule]
):
...
def create_module_from_leaves(leaves: tuple[Leaf, Leaf]):
    ...
    module = _LagoModule(C0_leaves)
    module.parent = parent_module

    return module


def get_neighbors_modules_parents(module: _LagoModule, time=True, topo=True):
    ...
    Returns:
        set: modules
    """
    neighbors_modules = set[_LagoModule]()
