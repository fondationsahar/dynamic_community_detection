...
from lago.enums import LexType
...
def longitudinal_modularity(
    ...
    communities: dict[CommunityLabel, set[tuple[int, int]]],
    lex_type: LexType = LexType.MM,
    alpha: float = 1.0,
    omega: float = 2.0,
    ndigits: int = 5,
) -> ModularityResult:
    ...
    Args:
        ...
        communities: Mapping from community label to set of (node, time) tuples.
        lex_type: Expectation type - LexType.CM, LexType.JM, or LexType.MM.
    ...
    Returns:
        ModularityResult containing the modularity value, time penalty, and lex_type.

    Raises:
        TypeError: If lex_type is not a LexType enum.
    ...
    Example:
        ...
        ```
    """
    # Validate lex_type is an enum
    if not isinstance(lex_type, LexType):
        raise TypeError(
            f"lex_type must be a LexType enum (LexType.CM, LexType.JM, or LexType.MM), "
            f"got {type(lex_type).__name__}"
        )

    # Build labels dict and communities data structures (NO modification to linkstream!)
    labels: LabelsDict = {}
    communities_leaves: dict[CommunityLabel, set[Leaf]] = {}
    communities_nodes: dict[CommunityLabel, set[int]] = {}  # Pre-computed for optimization
    ...
    if alpha == 0:
        ...
        communities_expectations = expectation_functions[lex_type](
