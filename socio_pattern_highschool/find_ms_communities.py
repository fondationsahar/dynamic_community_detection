...
from cd_methods import get_mm_communities, group_by_time
...
for agg_win in aggregation_windows:
    ...
    for interslice_weight_ratio in interslice_weight_ratios:
        print("\tinterslice_weight_ratio:", interslice_weight_ratio)
        n_tries_mm = 3
        mm_communities, _ = get_mm_communities(
            snapshots,
            interslice_weight_ratio=interslice_weight_ratio,
            n_tries=n_tries_mm,
        )

        pp_ms_communities = []
        for time_commu in mm_communities:
            tmp_time_commu = []
            ...
            pp_ms_communities.append(tmp_time_commu)

        with open(
            f"socio_pattern_highschool/commus_ms/ms_commu_groupby={agg_win}_iwr={interslice_weight_ratio}.json",
            "w",
        ) as file:
            json.dump(pp_ms_communities, file, indent=4)
