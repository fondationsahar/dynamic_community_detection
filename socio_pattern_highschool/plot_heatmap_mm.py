...
file_name = "socio_pattern_highschool/mm_commu_eval_emm.json"
file_name = "socio_pattern_highschool/mm_commu_eval_ejm.json"
...
for filename, results in data.items():
    ...
    data_to_plot[window_aggregation][iwr] = results["lm"]

df_data = pd.DataFrame.from_dict(data_to_plot).sort_index()
...
ax.set_ylabel("interslice weight ratio", fontsize=26)
...
plt.tight_layout()
plt.savefig(
    "socio_pattern_highschool/plots/lm_mme_ms_heatmap.png",
    dpi=300,
    bbox_inches="tight",
)
